...
这是一个针对 Microsoft Edge / Chromium 浏览器整理和修复后的 Neater Bookmarks 扩展版本。项目主体位于 `neater-bookmarks-fixed/` 目录，保留了原扩展的书签树弹窗、搜索、拖拽排序、右键菜单、选项页、自定义样式和多语言资源。
...
- 主要权限：书签、标签页、网页 favicon、全部 URL 访问
...
本项目是静态浏览器扩展源码，不需要构建步骤。修改 JavaScript、HTML、CSS 或 `_locales` 资源后，在 Edge 扩展管理页面点击扩展的“重新加载”即可测试。
